...
void DataAnalysisV1(Int_t ch, Int_t run, int fit, TString SignalFile)
{
  ...
  //smoothing + fitting
  if (ch > 15){ //plastic
    ADRIANOAmp1->Draw(Form("ADRIANOAmp1.ch%d>>htmp%d(100,-20.,2000.)", ch, ch), Form("ADRIANOTimeAmp1.ch%d > 21 && ADRIANOTimeAmp1.ch%d < 27 && ADRIANOPedSigma1.ch%d < 3", ch, ch, ch));
  }
  else { //glass
  ...
  cout << " htmp->GetEntries() " << htmp->GetEntries() << endl;
    if (ch > 15){
        htmp->Smooth(3);
    }
    ...
    if(fit){
      ...
      Double_t FitRangeA[2]   = {htmp->GetBinLowEdge(1), (ch==16)*400+(ch==17)*150.+0*htmp->GetBinLowEdge(nbins+1)}; //x-axis fit range
