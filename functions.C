...
void GetPedestalFEB(TArrayI w, double &ped, double &sigma, bool bShift=0)
{
  ...
  for(int i1=0;i1<w.GetSize()*0+20;i1++){
//     if(w.At(i1)>0.)
...
TF1 *langaufit(TH1F *his, Double_t *fitrange, Double_t *startvalues, Double_t *parlimitslo, Double_t *parlimitshi, Double_t *fitparams, Double_t *fiterrors, Double_t *ChiSqr, Int_t *NDF, int nloop = 1)//, )
{
  ...
  for (int nfit = 0; nfit < nloop-1; nfit++){
    his->Fit(FunName,"RB0MI");   // fit within specified range, use ParLimits, do not plot
  }
    his->Fit(FunName,"QRBMI");
...
TF1 *langaufit3(TH1F *his, Double_t *fitrange, Double_t *startvalues, Double_t *parlimitslo, Double_t *parlimitshi/*, Double_t *fitparams, Double_t *fiterrors, Double_t *ChiSqr, Int_t *NDF*/, Int_t nloops=1, Option_t *opt1="RBWI", Option_t *opt2="RBWI") //og QRB0WI
{
  // Once again, here are the Landau * Gaussian parameters:
  //   par[0]=Width (scale) parameter of Landau density
  //   par[1]=Most Probable (MP, location) parameter of Landau density
...
#endif
// fitsnr1->SetParameters(-10., -1.e6, -.1, 5.e-4, 20.);
